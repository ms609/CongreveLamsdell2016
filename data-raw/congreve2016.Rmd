...
nodelabels(c('', paste0(resamples$sym, '/', resamples$jak)))
ReduceTrees <- function(tree, nodeSupport) {
  nTip <- length(tree$tip.label)
  nNode <- tree$Nnode
  collapseOrder <- order(as.double(nodeSupport))
  reduced <- vector('list', length(nodeSupport))
  edge <- tree$edge
  nodeOrder <- nTip + collapseOrder
  for (i in seq_along(nodeOrder)) {
    node <- nodeOrder[i]
    parentEdge <- edge[, 2] == node
    parent <- edge[parentEdge, 1]
    edge[edge[, 1] == node, 1] <- parent
    edge <- edge[-parentEdge, ]
    newTree <- tree
    newTree$edge <- edge
    newTree$Nnode <- Nnode - i
    reduced[[i]] <- RenumberTree(newTree)
  }
}

ReduceTrees(resamples$tree, resamples$sym)
```
