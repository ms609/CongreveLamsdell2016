...
```{r eval=FALSE}
HOME <- "C:/Research/iw/" # Must end in a trailing '/'
sapply(paste0(HOME, c('', 'tntscript', 'S1-TNT', 'Trees')), dir.create)
```
...
unzip(tempFile, 'tnt.exe', exdir=HOME)
```
...
*	Download Appendix S1 from Congreve and Lamsdell [-@Congreve2016dd]  (doi:[10.5061/dryad.7dq0j/1](https://dx.doi.org/10.5061/dryad.7dq0j/1)) and unzip its 100 text files to S1-TNT
```{r eval=FALSE}
